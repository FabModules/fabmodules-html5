...
function mod_mesh_height_map(mesh,img) {
   ...
   //
   var rz = document.mod.mesh.rz
   var rx = document.mod.mesh.rx
   var dy = document.mod.mesh.dy
   var dx = document.mod.mesh.dx
   var s = document.mod.mesh.s
   var zlim = {zmin:1e10,zmax:-1e10}
   for (var t = 0; t < mesh.length; ++t)
      mod_mesh_height_triangle(mesh[t],img,rz,rx,dy,dx,s,zlim)
   document.mod.zmin = 25.4*(zlim.zmin-zlim.zmax)/document.mod.dpi
   //
   // set background
   //
   for (var row = 0; row < img.height; ++row)
...
function mod_mesh_height_triangle(t,img,rz,rx,dy,dx,s,zlim) {
   ...
   //
   function pos(v) {
      var x = 2*(v[0]-document.mod.mesh.xmin)/
         (document.mod.mesh.xmax-document.mod.mesh.xmin)-1
      var y = 2*(v[1]-(document.mod.mesh.ymax+document.mod.mesh.ymin)/2)/
         (document.mod.mesh.xmax-document.mod.mesh.xmin)
      var z = 2*(v[2]-(document.mod.mesh.zmax+document.mod.mesh.zmin)/2)/
         (document.mod.mesh.xmax-document.mod.mesh.xmin)
      var cz = Math.cos(rz)
      var sz = Math.sin(rz)
      var xrz = cz*x - sz*y
  ...
  var new_rule = ''
   for (var i = 0; i < rules[index].length; ++i) {
      switch (rules[index][i]) {
         case 'a': new_rule += 'c'; break;
         case 'b': new_rule += 'g'; break;
         case 'c': new_rule += 'k'; break;
         case 'd': new_rule += 'h'; break;
         case 'e': new_rule += 'd'; break;
         case 'f': new_rule += 'b'; break;
         case 'g': new_rule += 'j'; break;
         case 'h': new_rule += 'l'; break;
         case 'i': new_rule += 'a'; break;
         case 'j': new_rule += 'f'; break;
         case 'k': new_rule += 'i'; break;
         case 'l': new_rule += 'e'; break;
         case ' ': new_rule += ' '; break;
         }
      }
   ...
   function rotate_y(rules,index) {
     ...
     var new_rule = ''
      for (var i = 0; i < rules[index].length; ++i) {
         switch (rules[index][i]) {
            case 'a': new_rule += 'e'; break;
            case 'b': new_rule += 'd'; break;
            case 'c': new_rule += 'h'; break;
            case 'd': new_rule += 'l'; break;
            case 'e': new_rule += 'i'; break;
            case 'f': new_rule += 'a'; break;
            case 'g': new_rule += 'c'; break;
            case 'h': new_rule += 'k'; break;
            case 'i': new_rule += 'f'; break;
            case 'j': new_rule += 'b'; break;
            case 'k': new_rule += 'g'; break;
            case 'l': new_rule += 'j'; break;
            case ' ': new_rule += ' '; break;
            }
         }
      rules[new_index] = new_rule
   ...
   function rotate_z(rules,index) {
     ...
     for (var i = 0; i < rules[index].length; ++i) {
         switch (rules[index][i]) {
            case 'a': new_rule += 'b'; break;
            case 'b': new_rule += 'c'; break;
            case 'c': new_rule += 'd'; break;
            case 'd': new_rule += 'a'; break;
            case 'e': new_rule += 'f'; break;
            case 'f': new_rule += 'g'; break;
            case 'g': new_rule += 'h'; break;
            case 'h': new_rule += 'e'; break;
            case 'i': new_rule += 'j'; break;
            case 'j': new_rule += 'k'; break;
            case 'k': new_rule += 'l'; break;
            case 'l': new_rule += 'i'; break;
            case ' ': new_rule += ' '; break;
            }
         }
      rules[new_index] = new_rule
...
function mod_mesh_march_triangulate(min_threshold,max_threshold,buf,ptr,nx,ny,nz,z) {
   ...
   function triangulate_min() {
      ...
      //
      var rule = document.mod.mesh.rules[index]
      i = 0;
      while (i < rule.length) {
         if (rule[i] == ' ') {
            //
