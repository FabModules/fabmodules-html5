...
function mod_path_worker_find_edges(img) {
   ...
   for (var row = 1; row < (img.height-1); ++row) {
      ...
      if (img.get(row,col,STATE) & INTERIOR) {
         var sum =
              (img.get(row+1,col,STATE) & INTERIOR)
            + (img.get(row+1,col+1,STATE) & INTERIOR)
            + (img.get(row,col+1,STATE) & INTERIOR)
            + (img.get(row-1,col+1,STATE) & INTERIOR)
            + (img.get(row-1,col,STATE) & INTERIOR)
         if (sum != 5) {
...
function mod_path_worker_image_set_height(img,bottom_z,bottom_i,top_z,top_i,dpi) {
   ...
   for (var row = 0; row < img.height; ++row) {
      for (var col = 0; col < img.width; ++col) {
         var intensity = (img.get(row,col,0) + (img.get(row,col,1) << 8)
            + (img.get(row,col,B) << 16))/imax
...
function mod_path_worker_image_show_states(img) {
   img.get = function(row,col,element) {return this.data[(this.height-1-row)*this.width*4+col*4+element]}
   img.set = function (row,col,element,value) {this.data[(this.height-1-row)*this.width*4+col*4+element] = value}
   for (var row = 0; row < img.height; ++row) {
      for (var col = 0; col < img.width; ++col) {
   ...
   for (var row = 1; row < (img.height-1); ++row) {
      ...
      if (img.get(row,col,STATE) & EDGE) {
         if ((img.get(row,col+1,0) & INTERIOR) &&
           (!(img.get(row+1,col,0) & INTERIOR) ||
            !(img.get(row+1,col+1,0) & INTERIOR))) {
      ...
      if (img.get(row,col,STATE) & EDGE) {
         if ((img.get(row+1,col,0) & INTERIOR) &&
            !(img.get(row-1,col,0) & INTERIOR)) {
...
function mod_path_worker_sort_weighted(path,path_order,merge_distance,order_weight,sequence_weight) {
   ...
   while (path.length > 0) {
      ...
      for (var seg = 0; seg < path.length; ++seg) {
         ...
         var w = (x1-x0)*(x1-x0)+(y1-y0)*(y1-y0)
            + order_weight*path_order[seg]
            + sequence_weight*(seg/path.length)
         if (w < wmin) {
            wmin = w
            dmin = d
            var segmin = seg
