//
// mod_Roland_iModela_mill.js
//   fab modules Roland iModela mill output
//   for iModela only
//
// Neil Gershenfeld 
...
define(['require', 'handlebars', 'text!templates/mod_roland_mill_controls.html', 'mods/mod_ui', 'mods/mod_globals', 'mods/mod_file'], function(require) {
   ...
   if (input == "path (.svg)") {
      ...
      mod_add_process([
         ...
         ["routine", "mod_Roland_iModela_path"],
         ["command", "lpr -Pimodela"],
         ["depth", "0.1"],
         ["diameter", "0.4"],
         ["offsets", "4"],
         ["overlap", "50"],
   ...
   function mod_load_handler() {
      ...
      if (findEl('mod_move')) {
         findEl('mod_move').addEventListener("click", function() {
            var name = "move.rml";
            var xmin = 100 * parseFloat(findEl("mod_xmin").value);
            var ymin = 100 * parseFloat(findEl("mod_ymin").value);
   ...
   //
   // mod_Roland_iModela_path
   //    convert 3D path to RML
   //

   function mod_Roland_iModela_path(path) {
      globals.type = ".rml"
      ...
      var ijog = Math.floor(100 * jog) // 100/mm
      var scale = 100.0 * dx / (nx - 1) // 100/mm
      var xmin = parseFloat(findEl("mod_xmin").value)
      var ymin = parseFloat(findEl("mod_ymin").value)
      var xoffset = 100.0 * xmin // 100/mm
      var yoffset = 100.0 * ymin // 100/mm
      var zoffset = 0
   ...
   return {
      ...
      mod_Roland_iModela_path: mod_Roland_iModela_path
   }
